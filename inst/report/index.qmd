...
```{r }
# bag_api_url = covid19vaccinationchqmd:::bag_api_url
...
saveRDS(data1, file.path(data_path(), "data1.rds")) 
```
...
data1 <- readRDS(file.path(data_path(), "data1.rds")) 
...
saveRDS(data2, file.path(data_path(), "data2.rds")) 
...
#| context: server 
data2 <- readRDS(file.path(data_path(), "data1.rds")) 
# last_timeline_weeks <- readRDS("last_timeline_weeks.rds") 
...
file.remove(c(file.path(data_path(), "data1.rds"), file.path(data_path(), "data2.rds")))
...
This page article aims also at showing how an **R Markdown** file can be deployed to **shinyapps.io** to be a live page. For details on the architecture of the **covid19vaccinationchqmd** R package that supports this deployment and its installation see the [README](https://github.com/miraisolutions/covid19-vaccination-ch#readme){target="_blank"} on our public **GitHub** repository or read the related [**news post**](https://mirai-solutions.ch/news/2022/03/08/announce-covid19-vaccination-ch/){target="_blank"} where we described it more in details.
