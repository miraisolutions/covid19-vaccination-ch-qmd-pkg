...
subtitle: "An article published from a Quarto Document"
...
format:
  html:
    # theme: journal
    css: styles.css
    # mainfont: "Frutiger LT W01_55 Roma1475738"
    # fontsize: 18px
    toc: true
    toc-depth: 3
    toc-location: left
...
# 
table_css <- "border-collapse: separate; border-spacing: 0.25em; width: 100%; font-size: 0.6em; margin-bottom: 14.5px;border-collapse: false;" # original 14.5

# table_css <- "border-collapse: separate; border-spacing: 0.25em; display: inline-table; overflow-x: true; font-size: 0.5em; margin-bottom: 14.5px;border-collapse: false;" # original 14.5
...
To see how we read BAG data in **R** please refer to the [first article](https://mirai-solutions.ch/news/2021/10/20/covid19-vaccination-breakthroughs-switzerland/){target="_blank"}.
...
[^1]: *Please note that the data collection from BAG was sufficiently accurate until July 2022, therefore the current last 4 weeks report, and the historical comparisons from 2022Q3 onwards are less meaningful.*
...
As of **September 2022**[^2] the 4 last weeks considered are: **`r weeks4`** (YY-W-Week Number), i.e. in the interval from **`r weeks_from_to[1]`** to **`r weeks_from_to[2]`**.

[^2]: *The data collection and publication from BAG stopped in September 2022.*
...
TotConfirmed %>%
  ...
  setNames(replace(names(.), 1, "")) %>%
  addHtmlTableStyle(spacer.celltype = "skip",
...
A view of the absolute figures of all vaccination categories, including **"Unknown"**, i.e. not reported. [^3]

[^3]: *UPDATE 2022-07: as it can be seen from the large numbers in the Unknown category, BAG does not keep track properly any more of the vaccination status.*
...
DataTabScale %>%
  ...
  addHtmlTableStyle(spacer.celltype = "skip",
    align = "c", css.table = table_css, 
      css.cell = color_cells(DataTabScale %>% select(-1), header = "Status" , cgroup = "Case", rnames = "AgeClass", table_cell_css = table_cell_css)
  ) %>%
  tidyHtmlTable(header = Status ,
                cgroup = Case,
                rnames = AgeClass,
...
Even with some deficiency in the data, this analysis clearly shows the benefits of vaccination and what could have happened in Switzerland if vaccines had been unavailable.
...
If you have any question about the approach, or any suggestion for improvement please do not hesitate to get in [touch](https://mirai-solutions.ch/about/contact/){target="_blank"}, we would love to hear from you.

This page article aims also at showing how an **Quarto** file can be deployed to **shinyapps.io** to be a live page. For details on the architecture of the **covid19vaccinationch** R package that supports this deployment and its installation see the [README](https://github.com/miraisolutions/covid19-vaccination-ch-qmd-pkg#readme){target="_blank"} on our public **GitHub** repository or read the related [**news post**](https://mirai-solutions.ch/news/2022/03/08/announce-covid19-vaccination-ch/){target="_blank"} where we described it more in details.[^4]

[^4]: *This article is partially out of date, it describes the old version of the app, based on an R Markdown report, which was converted to Quarto in August 2023.*
